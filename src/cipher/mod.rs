use iced::{
    Element,
    widget::{TextInput, button, column, container, row, text},
};
...
impl Cipher {
    pub fn view(cipher: &Cipher) -> Element<'_, Message> {
        container(column![
            ...
            row![
                TextInput::new("keyword", &cipher.vigenere_keyword)
                    .on_input(Message::ContentChanged)
            ],
            row![
                TextInput::new("text to encrypt", &cipher.to_encrypt)
                    .on_input(Message::InputChanged)
            ],
            row![
                text("the caesar encrypted string: "),
                text(&cipher.caesar_encrypted)
            ],
            row![
                text("the vigenere encrypted string: "),
                text(&cipher.vigenere_encrypted)
            ]
        ])
        .into()
    }
